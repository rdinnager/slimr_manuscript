...
chromosome_sim <- slim_script(
  ...
  slim_block(initialize(), {
    ...
    initializeMutationType("m1", 0.5, "f", 0.0);
    initializeMutationType("m2", 0.5, "f", 0.1);
    # Convert to substitutions is false so we can keep tracking mutations after fixing.
    # There is a more efficient way to do this, but this is the easiest
    # for this demo
    initializeGenomicElementType("g1", c(m1, m2), c(10000, 1));
...
plot_HC <- function(positions, prevalence, chrs, bg, max_prev) {
  ...
  grDevices::dev.hold()
  hc = HilbertCurve(1, 262143, level = 4, reference = TRUE, arrow = FALSE)
  hc_polygon(hc, chrs)
