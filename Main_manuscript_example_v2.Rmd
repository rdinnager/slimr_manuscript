...
glimpse(snp_mat)
```
So this gives us something with the correct starting frequency of SNPs, but we also want our genome to have a realistic size. So we will estimate the total genome size for the part of the genome we sequenced by adding up all the sequence lengths of the alleles these SNPs came from, then we will randomly add a lot of zeroes to our SNP matrix. To save memory we will use a sparse matrix using the `Matrix` package. Then, we make our starting population file!

```{r make_pop_init}

#snp_mat <- Matrix(snp_mat)
#big_snp <- Matrix(nrow = nrow(snp_mat), ncol = genome_size)

#big_snp[ , sample.int(genome_size, ncol(snp_mat))] <- snp_mat
...
slim_make_pop_input(snp_mat, here("sims/starting_pop.txt"), ## filename 
                    ...
                    mut_pos = sample.int(default_genome_size - 1, nLoc(gen_2008)),   ## randomly assign snps
                                                                       ## to genome positions
...
slim_script(
  ...
  slim_function("o<Subpopulation>$ subpop1", "o<Subpopulation>$ subpop2",
                name = "calculateFST",
                return_type = "f$", body = !!rlang::fn_body(calculateFST)),
