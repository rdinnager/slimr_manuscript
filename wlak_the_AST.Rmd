...
##Abstract Syntax Trees
 ...
 I recently had reason to want to do something somewhat unusual. I wanted to take a R script, evaluate it, replacing all calls with their output. Sounds a little ctrange, but it can be handy if you are trying to make a domain specific language (DSL), where you can write R code that converts itself into, say, another language. It turns out you can learn a lot about how R does things under the hood bu attempting this sort of task, so I thought I'd share it in case anyone else finds it helful or at least interesting. Much of the following has been derived from a combination of reading Advanced R by Hadley Wickham, and a lot of experimentation. We will use the `rlang` package extensively, though it is possibly to similar things with base R, it will just be a little more difficult and probably more idosyncratic.
...
To walk the AST, we will write a recursive function that finds all calls, and then evaluates them, returning the results in a list that mirrors the nested structure of the original AST. It should end up having the same exact structure if it works correctly. Everything other than calls will be returned as is.
...
replace_and_eval <- function(x) {
  new_expr <- call_self_replace(x)
  rlang::eval_tidy(new_expr)
}
