...
## used to change simulation for SLiM version compatibility: should be 3 or 4
slim_vers <- 4
if(slim_vers == 4) {
  timer <- quote(community.tick)  
} else {
  timer <- quote(sim.generation)
}
...
slim_script(
  ...
  slim_block(1, 500, late(), {
    ...
    if(any(match(sample_these, !!timer) >= 0)) {
      ## find the sample size that matches the matching "year" for our obs data
      ssizes = drop(samp_sizes[ , which(sample_these == !!timer)]) 
      ## sample individuals
